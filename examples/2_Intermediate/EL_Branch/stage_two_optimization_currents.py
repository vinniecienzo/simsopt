...
rg = Generator(PCG64DXSM(0))
sigma = 5000
var = sigma**2
coils = coils_via_symmetries(base_curves, base_currents, s.nfp, True)
bs = BiotSavart(coils)
bs.set_points(s.gamma().reshape((-1, 3)))
J = SquaredFlux(s, bs).J()
print(f"J = {J}")
print([c.current.get_value() for c in coils])
Jprime = []
for i in range(500):
    coils_pert = [Coil(c.curve, CurrentPerturbed(c.current,sigma*rg.standard_normal())) for c in coils]
    bs_pert = BiotSavart(coils_pert)
    bs_pert.set_points(s.gamma().reshape((-1, 3)))
    Jprime.append(SquaredFlux(s, bs_pert).J())
print(f"<J'> = {np.mean(Jprime)}")
print(f"Theoretical <J'> = {J}")
print("Perturbed coil currents")
