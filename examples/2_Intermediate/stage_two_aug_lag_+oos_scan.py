...
the evaluation perturbation size over SIGMA_OOS_VALUES, holding the solution
fixed, so that evaluation size is decoupled from whatever size the solution was
designed against. SIGMA_OOS_SCAN = False reproduces the single-point behaviour
at SIGMA_OOS.
...
- Output files are saved in the './auglag/' directory for post-processing and visualization.
...
# Sweep the evaluation perturbation size against the one fixed solution. The
# optimization is unaffected; only the evaluation changes, so training and
# evaluation sizes are decoupled. Set False for a single point at SIGMA_OOS.
SIGMA_OOS_SCAN = True
...
for sigma_oos in sigma_oos_list:
    ...
    for i in range(N_OOS):
        ...
        base_curves_perturbed = [
            CurvePerturbed_jsonfix(c, PerturbationSample(sampler, randomgen=rg))
        ...
        coils_pert = [
            Coil(CurvePerturbed_jsonfix(c.curve, PerturbationSample(sampler, randomgen=rg)),
                 c.current)
            for c in coils_sys]
